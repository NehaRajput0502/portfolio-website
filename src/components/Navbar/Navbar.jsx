...
const Navbar = () => {
  ...
  return (
    <nav className="fixed top-0 left-0 w-full h-16 z-50 bg-[#000080] backdrop-blur-md shadow-md flex items-center justify-between px-8">
      <span className="text-3xl font-semibold italic font-serif drop-shadow-md text-[#B8B8FF] flex items-center gap-2">
        Neha Rajput
      </span>
      ...
      >
        {[
          ...
          <a href={`#${id}`} key={id} onClick={() => setMenuOpen(false)}>
            <li className="relative text-[#B8B8FF] text-xl font-bold px-4 py-2 rounded-lg transition-all duration-300 hover:text-purple-500 hover:bg-[rgba(251,239,251,0.8)] hover:-translate-y-1 
